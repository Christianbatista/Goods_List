...
public class SLL<E> {
	...
	private int findIndex(E targetData) {
		...
		SLLNode<E> cursor = this.head.getNext();
		while ((cursor != this.tail) &&
				(!(cursor.getNodeData().equals(targetData)))) { // Continue to loop as long as we have not made it to
																// the tail and the. first thing you are checking are
																// you at the tail. then i have not found the target
																// data
			cursor = cursor.getNext();
			++rv;
		} // Ending bracket of while loop
